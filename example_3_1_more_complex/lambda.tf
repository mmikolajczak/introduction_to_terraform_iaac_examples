...
resource "aws_lambda_function" "daily_cat_endpoint" {
  ...
  environment {
    variables = {
      ...
      "SAMPLING_STATISTICS_TABLE_NAME": aws_dynamodb_table.daily_cat_photos_sampling_statistics.name
    }
  }
}
  ...
  "Statement": [
      {
          ...
          "Resource": [
            "${aws_dynamodb_table.daily_cat_photos_sampling_statistics.arn}"
          ]
      }
  ]
}
